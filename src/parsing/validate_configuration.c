...
int validate_configuration(char **config_lines, int count, t_cub *cub)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (is_tex_id(id))
        {
            ...
            if (!ft_strcmp(id, "NO"))
            {
                if (cub->texture_paths[0]) { free(path); return 0; }
                cub->texture_paths[0] = path; have_no = 1;
            }
            else if (!ft_strcmp(id, "SO"))
            {
                if (cub->texture_paths[1]) { free(path); return 0; }
                cub->texture_paths[1] = path; have_so = 1;
            }
            else if (!ft_strcmp(id, "WE"))
            {
                if (cub->texture_paths[2]) { free(path); return 0; }
                cub->texture_paths[2] = path; have_we = 1;
            }
            else /* EA */
            {
                if (cub->texture_paths[3]) { free(path); return 0; }
        ...
        {
            if (*after == '\0') return 0; /* missing rgb */
